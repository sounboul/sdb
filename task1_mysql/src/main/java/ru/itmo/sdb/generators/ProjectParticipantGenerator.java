...
import ru.itmo.sdb.models.ProjectParticipantEntity;
...
public class ProjectParticipantGenerator implements Generator<ProjectParticipantEntity> {
    ...
    public ProjectParticipantEntity generate() {
        ...
        projectParticipant.setStartDate(
                new java.sql.Date(faker.date().past(360 * 5, TimeUnit.DAYS).getTime()
                ));
        projectParticipant.setFinishDate(new java.sql.Date(faker.date().between(
